...
public class AccessTokenRequest : MonoBehaviour {
	...
	IEnumerator getAccessToken(){
			...
			if (accessTokenRequest.exception != null)
				...
				Debug.Log(response);
				GameObject confirmBuy = GameObject.Find("Buy");
				confirmBuy.gameObject.transform.FindChild("SendObject").active = false;
				confirmBuy.gameObject.transform.FindChild("SendObject").active = true;
			}
	}
}
